...
def load_config():
    ...
    try:
        with open(CONFIG_FILE, "r") as f:
            ...
            required_fields = ["wallet_name", "initial_capital", "trading_pairs"]
...
def get_live_price(info, symbol):
    ...
    try:
        ...
        if symbol in all_mids:
            ...
            if symbol_data:
                # Get funding rate as proxy for 24h change
                funding = symbol_data.get("funding", "0")
                
                return {
                    "symbol": symbol,
                    "price": price,
                    "change_24h": float(funding) * 100,
                    "volume": 0,
                    "high_24h": price * 1.02,
                    "low_24h": price * 0.98
                }
...
def ai_analysis(pair, price_data, trend):
    ...
    try:
        ...
        if not api_key:
            log("⚠️  OPENAI_API_KEY not set - skipping AI analysis", "WARNING")
            return None
...
def execute_cycle():
    ...
    if not pairs:
        log("⚠️  No trading pairs configured", "WARNING")
        return
    ...
    for pair in pairs:
        ...
        if not analysis:
            log(f"⚠️  AI analysis skipped for {pair}")
            continue
